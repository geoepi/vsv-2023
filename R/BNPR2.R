# Functions here modified from phylodyn package, see https://github.com/mdkarcher/phylodyn
# add additional quantiles for plotting CIs.

BNPR2 <- function(data, lengthout = 100, pref=FALSE, prec_alpha=0.01,
                 ...
                 simplify = TRUE, derivative = FALSE, forward = TRUE, link=1)
{
  ...
  result <- infer_coal_samp2(samp_times = phy$samp_times, coal_times = phy$coal_times,
  ...
  result$effpop25  <- exp(-result$result$summary.random$time$`0.25quant`)
  result$effpop75  <- exp(-result$result$summary.random$time$`0.75quant`)
  ...
  result$summary <- with(result$result$summary.random$time,
                         data.frame(time = ID, mean = exp(-mean),
                                    ...
                                    quant0.25 = exp(-`0.25quant`),
                                    quant0.75 = exp(-`0.75quant`),
  ...
  if (derivative)
  {
    ...
    result$derivative <- with(result$result$summary.lincomb,
                              data.frame(time = result$x, mean = -mean[ind], sd = sd[ind],
                                         ...
                                         quant0.25 = -`0.25quant`[ind],
                                         quant0.5   = -`0.5quant`[ind],
                                         quant0.75 = -`0.75quant`[ind],
...
infer_coal_samp2 <- function(samp_times, coal_times, n_sampled=NULL, fns = NULL,
                             ...
                             prec_alpha = 0.01, prec_beta = 0.01, simplify = FALSE)
{
  ...
  mod <- INLA::inla(formula, family = "poisson", data = data, 
                    quantiles = c(0.025,0.25,0.5,0.75,0.975),
                    lincomb = lc_many,
